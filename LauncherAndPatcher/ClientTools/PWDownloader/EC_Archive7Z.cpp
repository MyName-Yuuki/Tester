...
static std::string GetFileExt(const char* filename)
{
	...
	if( pos != std::string::npos )
	{
		...
		std::transform(strExt.begin(), strExt.end(), strExt.begin(), tolower);	// תΪСд
		return strExt;
	}
	else
		return "";
				...
				stricmp(strFileName.c_str() + strFileName.length() - 4, ".pck") == 0 )
			{
				...
				if( fi.Find(fullProcessedPath) )
				{
					...
					if( PCK_HasKeyProtection(strFileName.c_str()) )
					{
						...
						{
							LogOutput("CExtractCallback::GetStream, PCK key verification FAILED for (%s)! Cannot update without correct key!", strFileName.c_str());
							MessageBoxA(NULL, "Key verification failed! Cannot update .pck file without correct key.", "PCK Protection Error", MB_OK|MB_ICONERROR);
							return E_ABORT;
						}
						LogOutput("CExtractCallback::GetStream, PCK key verification OK for (%s)", strFileName.c_str());
					}
				}
			}
			// ===== END PCK KEY VERIFICATION =====
...
bool CECArchive7Z::LoadPack(const char* filename, const char* password)
{
	...
	m_pStreamSpec = new CInFileStream;	// �������������ü�����ȻΪ0���㲻��
	m_pStreamSpec->AddRef();			// �����ֶ���Ϊ1
...
bool CECArchive7Z::ExtractTo(const char* szDestDir)
{
	...
	if( FAILED(hr) )
	{
		...
		MessageBoxA(NULL, msg, "����", MB_OK|MB_ICONERROR);
