...
bool AUITreeView::Tick(void)
{
	...
	if( (int)dwTick - m_nScrollDelay >= AUITREEVIEW_SCROLL_DELAY )
	{
...
bool AUITreeView::Render(void)
{
		...
		m_pParent->GetRenderState() == AUIDIALOG_RENDERSTATE_DYNAMIC && NeedDynamicRender() )
	{
		...
		for( i = nStart; i <= nEnd; i++ )
		{
			...
			pItem->rcItem.SetRect(rcText.left, y, x + nTextW, min(y + nItemHeight * pItem->nLines, rcText.bottom));
...
bool AUITreeView::AddVisibleItem(P_AUITREEVIEW_ITEM pItem)
{
	...
	if( m_pFont )
	{
		...
		if( m_bAutoWrap )
		{
			...
			if( m_pvecImageList && (*m_pvecImageList)[0] )
			{
				bval = AUI_TextOutFormat(m_pFont, 0, 0, pItem->strText,
					&rc, 0, &pItem->nLines, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, false, m_nShadow, 255, true, &pItem->itemsSet, m_pvecImageList, fWindowScale);
				if( !bval ) AUI_ReportError(__LINE__, 1, "AUIListBox::CalcTextLines(), failed to call AUI_TextOutFormat()");
			}
			else
			{
				bval = AUI_TextOutFormat(m_pFont, 0, 0, pItem->strText,
					&rc, 0, &pItem->nLines, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, false, m_nShadow, 255, true, &pItem->itemsSet, NULL, fWindowScale);
				if( !bval ) AUI_ReportError(__LINE__, 1, "AUIListBox::CalcTextLines(), failed to call AUI_TextOutFormat()");
			}
		}
		else
		{
			...
			float fWindowScale = m_pParent->GetAUIManager()->GetWindowScale() * m_pParent->GetScale();
			if( m_pvecImageList && (*m_pvecImageList)[0] )
				AUI_GetTextRect(m_pFont, pItem->strText, ptSize.x, ptSize.y, pItem->nLines, m_nLineSpace * fWindowScale, &pItem->itemsSet, 0, 0, m_pvecImageList, fWindowScale);
			else
				AUI_GetTextRect(m_pFont, pItem->strText, ptSize.x, ptSize.y, pItem->nLines, m_nLineSpace * fWindowScale);
...
void AUITreeView::PrintOptimizeResource()const
{
	for (int i = 0; i < AUITREEVIEW_RECT_NUM; i++)
	{
