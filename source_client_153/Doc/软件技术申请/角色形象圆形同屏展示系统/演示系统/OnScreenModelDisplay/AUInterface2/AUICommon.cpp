	...
	switch( idControl )
	{
		...
		case AUIOBJECT_TYPE_PROGRESS:
		{
			AUIOBJECT_PROPERTY p_add[] =
			{
				...
				{ "Pos Image File", AUIOBJECT_PROPERTY_SURFACE_FILENAME, AUI_pszFilterBmp },
...
#define FINISH_ENGLISHWORD {\
	if( bAutoReturn && pRect && bEnglishWord )\
	{\
		if( !bDrawEnglishWord )\
		{\
	...
	TEXTITEM_RCINFO* pItemRcInfo)
{
	...
	return textPrint.Output(inParams, outParams);
}

bool AUI_TextOutFormat(TAG_OP_IN_PARAMS& inParams, TAG_OP_OUT_PARAMS& outParams)
{
	return AUIRichTextPrinter::GetSingleton().Output(inParams, outParams);
}
...
ACString EditBoxItemBase::Serialize() const
{
	...
	if( m_type == enumEICoord )
	{
		swprintf(buf, L"<%d><%d><%d><%d><", m_type, m_dwColor, m_bUnderLine, m_bSameColor ? m_dwColor : m_dwUnderLineColor);
		str = buf + m_strName;
		str += L"><";
		str += m_strInfo;
	}
	else if( m_type == enumEIImage )
	{
		swprintf(buf, L"<%d><%d><%d><%d><%f><", m_type, m_dwColor, m_nImageIndex, m_nImageFrame, m_fImageScale);
		str = buf + m_strInfo;
	}
	else
	{
...
{
	const ACHAR* szNext = NULL;
	const ACHAR* szEnd = wcsstr(szText, _AL("<"));
	...
	{
		szNext = szEnd + 1;
		SetColor(_wtoi(szNext));
		szEnd = wcsstr(szNext, _AL("><"));
		if (szEnd == NULL)
			return false;

		szNext = szEnd + 2;				// szNext -> b_underline><cl_underline><name><info>
		m_bUnderLine = (_wtoi(szNext));
		szEnd = wcsstr(szNext, _AL("><"));
		if (szEnd == NULL)
			return false;

		szNext = szEnd + 2;
		m_dwUnderLineColor = (_wtoi(szNext));
		szEnd = wcsstr(szNext, _AL("><"));
		if (szEnd == NULL)
			return false;
		m_bSameColor = (m_dwUnderLineColor == m_dwColor);

		szNext = szEnd + 2;				// szNext -> name><info>
		szEnd = wcsstr(szNext, _AL("><"));
		if (szEnd == NULL)
			return false;

		SetName(ACString(szNext, szEnd - szNext));
		szEnd += 1;
	}
	else if( m_type == enumEIImage )
	{
	...
	else if( m_type == enumEIEmotion )
		SetName(_AL("W"));

	szNext = szEnd + 1;
	szEnd = wcschr(szNext, _AL('>'));
	...
	SetInfo(ACString(szNext, szEnd - szNext));
	szText = szEnd + 1;
	return true;
}
...
void glb_PrintOptimizeResource(A2DSprite *pA2DSprite)
{
	if(!pA2DSprite || !pA2DSprite->GetTextures())
	{
		return;
	...
	A2DSpriteTextures* pTex = pA2DSprite->GetTextures();
	a_LogOutput(1, "REF_COUNT:(%s)%d", pTex->GetName(), pTex->GetUserRef());
}
