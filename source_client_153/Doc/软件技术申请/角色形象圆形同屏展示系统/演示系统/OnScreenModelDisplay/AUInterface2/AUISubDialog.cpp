...
bool AUISubDialog::Tick(void)
{
	...
	if( (int)dwTick - m_nScrollDelay >= AUISUBDIALOG_SCROLL_DELAY )
	{
...
void AUISubDialog::PrintOptimizeResource()const
{	
	for (int i = 0; i < AUISUBDIALOG_RECT_NUM; i++)
	{
		if (m_pA2DSprite[i])
	...
	if (m_pVBarFrame)
		glb_PrintOptimizeResource(m_pVBarFrame->GetA2DSprite());
	
	if (m_pHBarFrame)
		glb_PrintOptimizeResource(m_pHBarFrame->GetA2DSprite());
	
	if (m_pSubDialog)
		m_pSubDialog->PrintOptimizeResource();
}
