	...
	int nAlpha, bool bNoFlush, int nShadow, bool bVertical)
{
	...
	if( m_nMode == MODE_1X1 )
	{
		...
		if( bNoFlush )
			bval = m_pA2DSprite->DrawToBack(xPos, yPos);
		else
			bval = m_pA2DSprite->DrawToInternalBuffer(xPos, yPos);
	}
	else
	{
...
//		ASSERT( nWidth >= W * 2 && nHeight >= H * 2 );
		...
		if( bNoFlush )
			bval = m_pA2DSprite->DrawToBack(x0, y0);
		else
			bval = m_pA2DSprite->DrawToInternalBuffer(x0, y0);
		if( !bval )
		{
			AString str;
		...
		if( m_nMode != MODE_1X3 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x1, y0);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x1, y0);
		...
		if( m_nMode != MODE_1X3 )
		{
			...
			m_pA2DSprite->SetScaleY(sy0);
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x2, y0);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x2, y0);
		...
		if( m_nMode != MODE_3X1 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x0, y1);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x0, y1);
		...
		if( m_nMode != MODE_3X1 && m_nMode != MODE_1X3 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x1, y1);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x1, y1);
		...
		if( m_nMode != MODE_3X1 && m_nMode != MODE_1X3 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x2, y1);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x2, y1);
		...
		if( m_nMode != MODE_3X1 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x0, y2);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x0, y2);
		...
		if( m_nMode != MODE_3X1 && m_nMode != MODE_1X3 )
		{
			...
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x1, y2);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x1, y2);
		...
		if( m_nMode != MODE_3X1 && m_nMode != MODE_1X3 )
		{
			...
			m_pA2DSprite->SetScaleY(sy2);
			if( bNoFlush )
				bval = m_pA2DSprite->DrawToBack(x2, y2);
			else
				bval = m_pA2DSprite->DrawToInternalBuffer(x2, y2);
	...
	if( pszText && a_strlen(pszText) > 0 )
	{
		...
		{	
			...
			pFont->TextOut(x + xOffset, y + yOffset, pszText, (nAlpha << 24) | (0xFFFFFF & color),
				nShadow ? true : false, A3DCOLORRGBA(0, 0, 0, nShadow * nAlpha / 255));
		}
		else
		{
