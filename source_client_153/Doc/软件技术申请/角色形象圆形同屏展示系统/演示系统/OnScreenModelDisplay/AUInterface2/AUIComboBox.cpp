	...
	if( WM_LBUTTONDOWN == msg || WM_LBUTTONDBLCLK == msg)
	{
		if( AUICOMBOBOX_STATE_NORMAL == m_nState )
		{
			...
			if( m_pListBox )
			{
				...
				{
					m_pListBox->AddString(m_Item[i].strText);
		...
		m_pParent->GetRenderState() == AUIDIALOG_RENDERSTATE_DYNAMIC && NeedDynamicRender() )
	{
		...
		if( m_Item.size() > 0 )
		{
			...
			bval = AUI_TextOutFormat(pFont, x, y, szColor + strText,
				&rc, 0, NULL, m_nLineSpace * fWindowScale, false, NULL, NULL, NULL, NULL, true, m_nShadow, nAlpha);
		...
		if( AUICOMBOBOX_STATE_POPUP == m_nState && IsFocus() && m_Item.size() > 0 && !m_pListBox )
		{
			...
			for( int i = 0; i < int(m_Item.size()); i++ )
			{
					...
					m_nAlign == AUIFRAME_ALIGN_CENTER )
				{
					...
					AUI_GetTextRect(pFont, m_Item[i].strText, nWidth, nHeight, nLines, 3);
					if( m_nAlign == AUIFRAME_ALIGN_CENTER )
...
void AUIComboBox::PrintOptimizeResource()const
{
	glb_PrintOptimizeResource(m_pA2DSpriteHilight);	
	if (m_pAUIFrame)
		glb_PrintOptimizeResource(m_pAUIFrame->GetA2DSprite());	
	if (m_pAUIFrameExpand)
		glb_PrintOptimizeResource(m_pAUIFrameExpand->GetA2DSprite());	
	if (m_pAUIFramePushed)
		glb_PrintOptimizeResource(m_pAUIFramePushed->GetA2DSprite());
}