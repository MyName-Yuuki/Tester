...
bool AUIManager::Render(void)
{
	...
	while( pos )
	{
		...
		if( pDlg && pDlg->IsShow() && pDlg != m_pDlgHint)
		{
...
bool AUIManager::DealWindowsMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		if( m_pDlgActive && m_pDlgActive->IsShow() && m_pDlgActive->IsEnabled())
		{
	...
	else if( GetWindowMessageType(uMsg) == WINDOWMESSAGE_TYPE_MOUSE )
	{
			...
			!bFocusComboBox) )
		{
			...
			while( pos )
			{
				...
				AUIDialog* pDlgParent = pDlg;
				while( pDlgParent->GetParentDlgControl() )
					pDlgParent = pDlgParent->GetParentDlgControl()->GetParent(); //子窗口有可能大小超过父窗口
				
				if( !pDlgParent->GetRect().PtInRect(x, y) || !pDlg->IsShow() || !pDlg->IsEnabled()) continue;
		...
		if( pDlgOnMsg && pDlgOnMsg->IsEnabled() && 
