...
#ifdef _ANGELICA22
static const char* cs_sdrfile = "Shaders\\2.2\\Progress.sdr";
#else
...
bool AUIProgress::InitXML(A3DEngine *pA3DEngine, A3DDevice *pA3DDevice, AXMLItem *pItem)
{
	...
	if (!m_strPosBgImage.IsEmpty() && !m_strPosImage.IsEmpty())
	{
...
void AUIProgress::PrintOptimizeResource()const
{
	if (m_pA2DSpriteBar)
		glb_PrintOptimizeResource(m_pA2DSpriteBar);
	
	if (m_pA2DSpritePos)
		glb_PrintOptimizeResource(m_pA2DSpritePos);
	
	if (m_pA2DSpriteInc)
		glb_PrintOptimizeResource(m_pA2DSpriteInc);
	
	if (m_pA2DSpriteDec)
		glb_PrintOptimizeResource(m_pA2DSpriteDec);
