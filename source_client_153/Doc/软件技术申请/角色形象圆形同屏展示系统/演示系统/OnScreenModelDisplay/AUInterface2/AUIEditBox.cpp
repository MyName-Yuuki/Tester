...
static void valid_number_str(ACString& strText)
{
	...
	if (iValidIndex < iLen)
		strText[iValidIndex] = '\0';

	// ACString.GetLength seems like not changed when string'content modified by [] operation. so... ugly but works
...
void AUIEditBox::SetText(const ACHAR *pszText)
{
	...
	if( m_szText.GetLength() > m_nMaxLen )
		m_szText.CutRight(m_szText.GetLength() - m_nMaxLen);

	SetDynamicRender(false);
	m_ItemsSet.Release();
	m_bHasItem = false;
	m_szBuffer = UnmarshalEditBoxText(pszText, m_ItemsSet);
	int nBufLen = m_szBuffer.GetLength();
	int nTextLen = m_szText.GetLength();
	m_nMaxLen = m_nMaxLenOrg - (nTextLen - nBufLen);
	m_xEndPos = nBufLen;
...
void AUIEditBox::SetTextAndItem(const ACHAR *pszText, const ACHAR* szIvtrItem, A3DCOLOR clIvtrItem, const EditboxScriptItem** ppScriptItems, int nScriptItemCount)
{
	...
	m_ItemsSet.Release();
	m_bHasItem = false;
	m_szBuffer = UnmarshalEditBoxText(pszText, m_ItemsSet, 0, szIvtrItem, clIvtrItem, 0xffffffff, ppScriptItems, nScriptItemCount);
	int nBufLen = m_szBuffer.GetLength();
	int nTextLen = m_szText.GetLength();
	m_nMaxLen = m_nMaxLenOrg - (nTextLen - nBufLen);
	m_xEndPos = nBufLen;
...
bool AUIEditBox::OnDlgItemMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		...
		case WM_KEYDOWN:
			OnKeyDown(wParam);
			return !AUI_PRESS(VK_CONTROL) && !AUI_PRESS(VK_MENU) && wParam != VK_TAB;
		case WM_KEYUP:
			return !AUI_PRESS(VK_CONTROL) && !AUI_PRESS(VK_MENU);
...
bool AUIEditBox::Tick(void)
{
	if( m_bMultiline && m_pA2DSprite[AUIEDITBOX_RECT_SCROLL] )
	{
		...
		if( (int)dwTick - m_nScrollDelay >= AUIEDITBOX_SCROLL_DELAY )
		{
...
void AUIEditBox::SetIsReadOnly(bool bReadOnly)
{
...
void AUIEditBox::SetIsMultipleLine(bool bMultiLine)
{
...
int AUIEditBox::InsertCompStr(bool bResult)
{
	...
	m_szBuffer = m_szBuffer.Left(m_xInsertPos) + m_szComStr	+ m_szBuffer.Right(nTrail);
...
//		return;
	...
	default :
		...
		szInsert[0] = wChar;
		szInsert[1] = 0;
		m_szBuffer = m_szBuffer.Left(m_xInsertPos) + szInsert
			+ m_szBuffer.Right(m_szBuffer.GetLength() - m_xInsertPos);
		//memcpy(szTmpStr, (const wchar_t *)m_szBuffer + m_xInsertPos, WCHARSIZE * (m_xEndPos - m_xInsertPos));
...
void AUIEditBox::OnKeyDown(UINT nChar) 
{
	...
	if (nChar == VK_RETURN && !m_bMultiline)
		if( m_strCommand != "" )
			m_pParent->SetCommand(m_strCommand);
		else
...
bool AUIEditBox::AppendItem(EditboxItemType type, A3DCOLOR cl, const ACHAR* szName, const ACHAR* szInfo, const void* pExtraData, size_t sz)
{
	...
	int nItemLen = pItem->Serialize().GetLength();

	if (nBufLen + nItemLen >= m_nMaxLen)
	{
	...
	m_nMaxLen -= nItemLen;
	m_szBuffer = m_szBuffer.Left(m_xInsertPos) + ch	+ m_szBuffer.Right(m_szBuffer.GetLength() - m_xInsertPos);
	m_xEndPos++;
	m_xInsertPos++;
	m_xCaretPos++;
	CalcTexWidthBuf();
...
void AUIEditBox::PrintOptimizeResource()const
{
	for (int i = 0; i < AUIEDITBOX_RECT_MAX; i++)
	{
		...
		if( OpenClipboard(m_pA3DDevice->GetDeviceWnd()) )
		{
			...
			if( hHandle )
			{
				...
				if( pszText && (m_nMaxLen < 0 || m_szBuffer.GetLength() < m_nMaxLen) )
				{
					...
					if( m_nMaxLen >= 0 && m_szBuffer.GetLength() + nLen > m_nMaxLen )
					{
						strText.CutRight(m_szBuffer.GetLength() + nLen - m_nMaxLen);
						nLen = m_nMaxLen - m_szBuffer.GetLength();
					}
					...
					m_szBuffer = m_szBuffer.Left(m_xInsertPos) + strText
						+ m_szBuffer.Right(m_szBuffer.GetLength() - m_xInsertPos);
					if( !m_bMultiline )
					{
						for(int i = m_xInsertPos; i < nLen + m_xInsertPos; i++ )
							if( m_szBuffer[i] == '\r' || m_szBuffer[i] == '\n' )
								m_szBuffer[i] = ' ';
					}
					m_xEndPos += strText.GetLength();
					m_xInsertPos += strText.GetLength();
					m_xCaretPos += strText.GetLength();
					m_szText = m_szBuffer;
				}					
...
void AUIEditBox::DoAlign()
{
	...
	while (i < nText)
	{
		...
		case AUIEDITBOX_ALIGN_CENTER:
			nAlignOffset = (m_rcText.Width() - (m_CharWidthBuf[idxLineEnd]-m_CharWidthBuf[idxLineBeg])) / 2;
			break;
		case AUIEDITBOX_ALIGN_RIGHT:
			nAlignOffset = m_rcText.Width() - (m_CharWidthBuf[idxLineEnd]-m_CharWidthBuf[idxLineBeg]);
			break;
		}
