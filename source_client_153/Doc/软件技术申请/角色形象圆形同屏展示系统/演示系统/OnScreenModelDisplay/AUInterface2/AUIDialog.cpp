...
void AUIDialog::PrintOptimizeResource()const
{
	glb_PrintOptimizeResource(m_pA2DSpriteHilight);	
	...
	if (m_pAUIFrameDlg)
		glb_PrintOptimizeResource(m_pAUIFrameDlg->GetA2DSprite());
	
	if (m_pAUIFrameTitle)
		glb_PrintOptimizeResource(m_pAUIFrameTitle->GetA2DSprite());
	...
	while (pElement)
	{
		pElement->pThis->PrintOptimizeResource();
		pElement = pElement->pNext;
	}
}
...
bool AUIDialog::RenderOnce(void)
{
	...
	if( m_nRenderState != AUIDIALOG_RENDERSTATE_DYNAMIC )
	{
		...
		if( !m_pParentDlgControl && m_pAUIFrameDlg )
		{
...
bool AUIDialog::IsInHitArea(int x, int y)
{
	...
	if( m_pParentDlgControl || !m_bAnomaly )
		return true;
...
void AUIDialog::AddFrame(A3DViewport *pViewport, A3DRECT &rc)
{
	FRAME frame;
...
void AUIDialog::ChangeFrame(A3DViewport *pViewport, A3DRECT &rc)
{
	int i;
