					...
					if( nCurSel >= 0 && nCurSel < int(m_Item.size()) )
					{
						if( m_bMultipleSelection )
						{
							if( WM_LBUTTONDOWN == msg || msg == WM_RBUTTONUP )
							{
								...
								{
									BatchSetSel(0, m_Item.size() - 1, false);
...
bool AUIListBox::Tick(void)
{
	...
	if( (int)dwTick - m_nScrollDelay >= AUILISTBOX_SCROLL_DELAY )
	{
		bool bDelay = true;
...
bool AUIListBox::Render(void)
{
		...
		m_pParent->GetRenderState() == AUIDIALOG_RENDERSTATE_DYNAMIC && NeedDynamicRender() )
	{
		...
		if( m_Item.size() > 0 && m_pA2DSprite[AUILISTBOX_RECT_ITEM] )
		{
			...
			float fScaleY = float(nTextH) / float(m_pA2DSprite[AUILISTBOX_RECT_ITEM]->GetHeight());			
			...
			{
				if( i > nStart )
					y += nTextH * m_Item[i - 1].nLines;
				
				if( m_bAutoWrap )
				{
					float fScaleY = min(float(nTextH * m_Item[i].nLines),
						rcText.bottom - y) / float(m_pA2DSprite[AUILISTBOX_RECT_ITEM]->GetHeight());
					m_pA2DSprite[AUILISTBOX_RECT_ITEM]->SetScaleY(fScaleY);
				}
				else
				{
					if (rcText.bottom - y < nTextH)
					{
		...
		if( m_Item.size() > 0 && m_pA2DSprite[AUILISTBOX_RECT_HILIGHT] )
		{
			...
			float fScaleY = float(nTextH) / float(m_pA2DSprite[AUILISTBOX_RECT_HILIGHT]->GetHeight());
		...
		for( i = nStart; i <= nEnd; i++ )
		{
			...
			if( m_nNumColumns > 1 )
			{
				...
				for( int j = 0; j < nNumCols; j++ )
				{
					if (m_bRenderHScroll)
					{
						...
						rc.SetRect(l, y, r, min(y + nTextH * m_Item[i].nLines, rcText.bottom));
					}
					else
					{
						rc.SetRect(x, y, x + m_nColWidth[j] * fWindowScale, min(y + nTextH * m_Item[i].nLines, rcText.bottom));
					}
					...
					if (m_bRenderHScroll)
						bval = AUI_TextOutFormat(pFont, x - m_nHBarPos, y, szColor + vec[j],
							&rc, 0, NULL, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, true, m_nShadow, nAlpha, true, &m_Item[i].itemsSet, m_pvecImageList, fWindowScale,
							true, NULL, 0, &rcInfo);
					else
						bval = AUI_TextOutFormat(pFont, x, y, szColor + vec[j],
							&rc, 0, NULL, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, true, m_nShadow, nAlpha, true, &m_Item[i].itemsSet, m_pvecImageList, fWindowScale,
							true, NULL, 0, &rcInfo);
			...
			else
			{
				if( m_bAutoWrap )
				{
					...
					bval = AUI_TextOutFormat(pFont, x - m_nHBarPos, y, szColor + m_Item[i].strText,
						&rc, 0, &m_Item[i].nLines, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, true, m_nShadow, nAlpha, true, &m_Item[i].itemsSet, m_pvecImageList, fWindowScale,
						true, NULL, 0, &rcInfo);
					if( !bval ) return AUI_ReportError(__LINE__, 1, "AUIListBox::Render(), failed to call AUI_TextOutFormat()");
				}
				else
				{
					rc.SetRect(x, y, rcText.right, y + nTextH);
		...
		for( int j = 0; j < nNumCols; j++ )
		{
			if( m_nColAlign[j] == AUIFRAME_ALIGN_LEFT && m_bAutoWrap)
			{
				...
				bool bval = AUI_TextOutFormat(pFont, 0, 0, vec[j],
					&rc, 0, &nLines, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, false, m_nShadow, 255, true, &m_Item[nIndex].itemsSet, m_pvecImageList, fWindowScale);
				if( !bval ) 
	...
	else
	{
		if (m_bAutoWrap)
		{
			...
			AUI_TextOutFormat(pFont, 0, 0, m_Item[nIndex].strText,
				&rcText, 0, &m_Item[nIndex].nLines, m_nLineSpace * fWindowScale, true, NULL, NULL, NULL, NULL, false, m_nShadow, 255, true, &m_Item[nIndex].itemsSet, m_pvecImageList, fWindowScale);
		}
	}
}
...
void AUIListBox::CalcWrapScroll(bool bCalcAllLines)
{
	...
	int cx = sLimit.cx / 2;
	int cy = sLimit.cy / 2;
	int nLinesPP = GetLinesPerPage(m_nHeight - cy * 2);
...
void AUIListBox::PrintOptimizeResource()const
{
	for (int i = 0; i < AUILISTBOX_RECT_NUM; i++)
	{
	...
	else
	{
		...
		else
		{
			...
			if( m_pvecImageList && (*m_pvecImageList)[0] )
				...
				&m_Item[nIndex].itemsSet, 0, 0, m_pvecImageList, fWindowScale, NULL, true);
			else
				AUI_GetTextRect(pFont, m_Item[nIndex].strText, ptSize.x, ptSize.y, m_Item[nIndex].nLines, m_nLineSpace * fWindowScale,
				NULL, 0, 0, NULL, fWindowScale, NULL, true);
...
int AUIListBox::GetLinesPerPage(int nPageHeight/*=-1*/)
{
	int nLinesPP = 0;
	float fWindowScale = m_pParent->GetScale() * m_pParent->GetAUIManager()->GetWindowScale();
	int nTextH = GetLineHeight();
	int nHeight = nPageHeight <= 0 ? m_rcImage[AUILISTBOX_RECT_FRAME].Height() : nPageHeight;
	nLinesPP = (nHeight + m_nLineSpace * fWindowScale) / nTextH;
