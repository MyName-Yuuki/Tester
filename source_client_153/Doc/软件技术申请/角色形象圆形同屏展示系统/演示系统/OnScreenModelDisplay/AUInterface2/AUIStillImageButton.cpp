		...
		else if( m_szText.GetLength() > 0 && !(m_bBlinkText && m_bBlink) )
		{
			...
			if (!m_bVerticalText)
				pFont->TextOut(x + m_nTextOffsetX * fWindowScale, y + m_nTextOffsetY * fWindowScale, m_szText, (nAlpha << 24) | (0xFFFFFF & color),
					m_nShadow ? true : false, A3DCOLORRGBA(0, 0, 0, m_nShadow * nAlpha / 255));
			else
...
{
	if( m_pAUIFrameOnHover && m_bHover != bHover )
		UpdateRenderTarget();
...
void AUIStillImageButton::PrintOptimizeResource()const
{	
	if (m_pAUIFrameUp)
		glb_PrintOptimizeResource(m_pAUIFrameUp->GetA2DSprite());
	
	if (m_pAUIFrameDown)
		glb_PrintOptimizeResource(m_pAUIFrameDown->GetA2DSprite());
	
	if (m_pAUIFrameOnHover)
		glb_PrintOptimizeResource(m_pAUIFrameOnHover->GetA2DSprite());
	
	if (m_pAUIFrameDisabled)
		glb_PrintOptimizeResource(m_pAUIFrameDisabled->GetA2DSprite());
}
