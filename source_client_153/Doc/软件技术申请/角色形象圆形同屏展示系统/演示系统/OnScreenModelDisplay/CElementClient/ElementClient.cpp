...
bool prepare_file_structures()
{
	...
	if( _access(szLogPath, 0) == -1 )
	{
		...
		{
			MessageBoxA(NULL, "无法创建logs目录，软件无法运行！\n请确保硬盘有足够的剩余空间再运行本软件！", "错误", MB_OK | MB_ICONSTOP);
			return false;
		}
	}
...
bool CheckHardwareCaps(const char *& pszInfo)
{
	...
	{
		MessageBoxA(NULL, "软件程序检测到您的机器上有NetMeeting正在运行！\n"
			"由于NetMeeting运行时其他程序无法使用显卡的三维加速功能，所以请先关闭NetMeeting 后再进行软件！", 
	...
	if( dwLocalTotal < 16 * 1024 * 1024 )
	{
		if( iTexDetail != 2 )
		{
			sprintf(szInfo, "您的显卡的显存小于16M，而您的贴图精度选择了[%s]，这将使您在软件中很卡！建议您将贴图精度调低后再进行软件！\n\n是否让系统自动选择合适的贴图精度来进行软件？", szTexQuality[iTexDetail]);
	...
	else if( dwLocalTotal < 32 * 1024 * 1024 )
	{
		if( iTexDetail != 2 )
		{
			sprintf(szInfo, "您的显卡只有32M显存，而您的贴图精度选择了[%s]，这将使您在软件中很卡！建议您将贴图精度调低后再进行软件！\n\n是否让系统自动选择合适的贴图精度来进行软件？", szTexQuality[iTexDetail]);
			if( IDYES == MessageBoxA(NULL, szInfo, "贴图精度过高", MB_YESNO | MB_ICONQUESTION) )
			{
				g_GameCfgs.GetSystemSettingsPtr()->iTexDetail = 2;
			}
	...
	else if( dwLocalTotal < 64 * 1024 * 1024 )
	{
		if( iTexDetail == 0 )
		{
			sprintf(szInfo, "您的显卡有64M显存，而您的贴图精度选择了[%s]，这将使您在软件中很卡！建议您将贴图精度调低后再进行软件！\n\n是否让系统自动选择合适的贴图精度来进行软件？", szTexQuality[iTexDetail]);
			if( IDYES == MessageBoxA(NULL, szInfo, "贴图精度过高", MB_YESNO | MB_ICONQUESTION) )
			{
	...
	{
		sprintf(szInfo, "您的系统剩余的可用于贴图的显存小于16M，在软件的过程中可能会导致花屏、死机等严重的错误！您是否还要进入软件？");
		if( IDYES != MessageBoxA(NULL, szInfo, "贴图精度过高", MB_YESNO | MB_ICONQUESTION) )
		{
			pszInfo = "系统剩余的可用于贴图的显存小于16M，用户取消了软件运行。";
			return false;		
		}		
	}
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, ACHAR* lpCmdLine, int nCmdShow)
{
	...
	{
		MessageBoxA(NULL, szInfo, "对不起，由于您的系统硬件或系统软件的问题，无法运行本软件!", MB_OK | MB_ICONSTOP);
#ifdef ENABLE_UNKNOWN_EXIT_CHECK
	...
	{
		MessageBoxA(NULL, "软件资源缺少重要文件！", szMissedFile, MB_OK | MB_ICONSTOP);
#ifdef ENABLE_UNKNOWN_EXIT_CHECK
		l_bEnableExitCheck = false;
#endif
		return -1;
...
bool _InitGameApp()
{
	...
	for(int i=0; i<sizeof(g_szPckDir) / sizeof(const char*); i++)
	{
		...
		if( !g_AFilePackMan.OpenFilePackageInGame(szPckFile) ){
			a_LogOutput(1, "_InitGameApp(), Failed to open package %s", szPckFile);
		}
	}
...
LRESULT CALLBACK _WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if( g_pGame )
	{
			...
			b)
		{
			...
			//	用于处理软件内置 WebBrowser 控件抢占焦点导致软件主窗口显示为失去焦点的问题
			b = false;
			wParam = TRUE;
		}
	}
