...
public:
	...
	union
	{
		bool    m_Bool;
		double  m_Number;
		DWORD   m_UserData; //pointer,  not set an int
		__int64 m_64BitsId;
	};
	...
	void    RetrieveAString(AString& out)
	{
		assert(m_Type == SVT_STRING);
		m_String.RetrieveAString(out);
	}

	void    RetrieveAWString(AWString& out)
	{
		assert(m_Type == SVT_STRING);
		m_String.RetrieveAWString(out);
	}
	
	void RetrieveUtf8(AString& out)
	{
		assert(m_Type == SVT_STRING);
